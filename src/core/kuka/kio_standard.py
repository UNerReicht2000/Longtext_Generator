...
def create_longtext(in_data,config,delet_marker,dat_typ,BufferInstanz):  
    BufferInstanz.append('created according to KIO standard')

    longtext = []
    
    current_directory = path.dirname(__file__)
    file_path = path.join(current_directory, 'emtpy_longtext.csv')
    longtext_dat = open(file_path, 'r')      
    longtext = longtext_dat.read()
    longtext = longtext.split("\n")

    black_list = ['Decl','Global','Const','Int','Signal ','Bool','Real','Char']

    for index in range(len(in_data)):
        in_data[index] = in_data[index].lstrip()
        for item in black_list:
            in_data[index] = in_data[index].replace(item,'')
            in_data[index] = in_data[index].replace(item.upper(),'')
            in_data[index] = in_data[index].replace(item.lower(),'')
        in_data[index] = delete_comment(in_data[index],';')   
        in_data[index] = in_data[index].strip()
    in_data = delete_content(in_data,'') 

    start_index_cycflag = 1219
    start_index_input = 1539
    start_index_output = 5635
    ...
    for config_item in config:
        ...
        if (config_item == 40):
            ...
            longtext = add_in_longtext(find_singel_objects(in_data,'CYC',delet_marker,BufferInstanz),longtext,1219,'$CYC_FLAG[',dat_typ)
            pass
        if (config_item == 50):
        ...
        if (config_item == -500):
            ...
            longtext = add_in_longtext(find_group_objects(in_data,'CO',delet_marker,BufferInstanz),longtext,5635,'OUT[',dat_typ)
...
def find_singel_objects(in_data,scan,delet_marker,BufferInstanz):
    ...
    if found_something: 
        ...
        for index in range(len(singel_objects)):
            ...
            #     singel_objects[index][sub_index] = singel_objects[index][sub_index].strip()
      
        for index in range(len(singel_objects)):
            if (len(singel_objects[index]) == 2):
                singel_objects[index][0] = singel_objects[index][0].strip()
                singel_objects[index][1] = singel_objects[index][1].strip()
